...
const SEED_DOCS = [
  ...
  {
    text: "Code Guard (src/security/code-guard.js): runtime injection detection that intercepts malicious code BEFORE it executes. Three layers: (1) Module._load hook — scans every require() call against 33 static analysis patterns before Node.js runs the code. Critical threats return empty module, non-critical warn. (2) File watcher — monitors server/ and wolverine-claw/ for new/modified .js files in real-time via fs.watch. (3) Execution boundary — blocks ALL require() from outside the project root (prevents /tmp escape attacks). 33 patterns in 10 categories: eval/Function injection, command injection (execSync/spawn with user input), dynamic require (path traversal), prototype pollution (__proto__), fs write/delete injection, SQL concatenation, SSRF (fetch/axios with user input), reverse shells (spawn /bin/sh), environment manipulation, obfuscation (charCode 10+ chars, base64 50+ chars, hex/unicode sequences), dynamic Function constructor. When injection detected: code blocked (empty module returned), file quarantined to .wolverine/security/quarantine/, forensic log written (full code, SHA-256 hash, stack trace, timestamp) to injection-log.jsonl, attack vector traced via call stack, IPC alert to parent process, self-improvement loop triggered. Lockdown mode available: blocks ALL new file loads during active attack. Zero false positives verified against production server code (23/23 tests pass).",
    metadata: { topic: "code-guard" },
  },
  {
    text: "Self-Improvement Pipeline (src/security/self-improve.js): learns from injection attacks using a DETECT→ANALYZE→HARDEN→VERIFY→APPLY loop. (1) DETECT: code-guard catches injection and emits event. (2) ANALYZE: AI examines attack code + entry vector using cheapest model (CLASSIFIER_MODEL). Returns attackType, entryVector, severity, hardening recommendations, confidence score. (3) HARDEN: generates defense recommendations (add-pattern, block-route, add-validation, update-config). (4) VERIFY: safe vs risky classification — risky changes deferred to human approval. (5) APPLY: auto-applies safe fixes, max 3 per hour (prevents runaway loops). Full audit trail in .wolverine/security/improvement-log.jsonl. Human notification via IPC for pending approvals. Adapted from Ralph autonomous agent loop pattern with dual-gate exit conditions and circuit breaker safety nets.",
    metadata: { topic: "self-improve" },
  },
  {
